...
app.layout = html.Div(style={'fontFamily': 'Arial', 'backgroundColor': colors['background'], 'padding': '20px'}, children=[
    html.H1('Fraud Detection Dashboard (INR)', style={'textAlign': 'center', 'color': colors['text']}),
...
def update_output(contents):
    if contents is not None:
        ...
        try:
            ...
            # Format Amount column to INR if exists
            ...
            # Scale the features (remove currency symbol before scaling)
            X_scaled_input = df.copy()
            if 'Amount' in X_scaled_input.columns:
                X_scaled_input['Amount'] = X_scaled_input['Amount'].replace('[₹, ]', '', regex=True).astype(float)

            X_new_scaled = scaler.transform(X_scaled_input[expected_columns])
            ...
            pie_chart = dcc.Graph(
                figure=go.Figure(
                    ...
                    layout=go.Layout(title='Prediction Distribution (INR)', title_x=0.5)
                )
            )
            ...
            elements = [
                ...
                dash_table.DataTable(
                    ...
                    columns=[{'name': ('₹ Amount' if i == 'Amount' else i), 'id': i} for i in X_new.columns],
                    page_size=10,
            ...
            # Extra analysis if labels exist
